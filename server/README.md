...
Backend API для интернет-магазина AURA на Express.js + SQLite.
...
Эта команда:
- Создаст SQLite базу данных в `server/database/aura.db`
- Выполнит все миграции (создание таблиц)
...
│   └── aura.db         # SQLite файл (создается автоматически)
├── migrations/
...
### SQLite
- Легковесная БД, не требует установки
- Вся БД хранится в одном файле: `server/database/aura.db`
- Идеально для разработки и небольших проектов
- Легко мигрировать на PostgreSQL в будущем

## 🔄 Миграция на PostgreSQL

Когда потребуется PostgreSQL:

1. Установите PostgreSQL
2. Создайте базу данных
3. Замените `sqlite3` на `pg` в `package.json`
4. Обновите `database/db.js` для работы с PostgreSQL
5. Адаптируйте SQL запросы (минимальные изменения)
